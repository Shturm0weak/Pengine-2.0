    ...
    public:
            ...
            const std::string& propertyName, size_t offset)
        {
            ...
            if (classByType != ReflectionSystem::GetInstance().m_ClassesByType.end())
            {
                ...
                {
                    if (SetValueImpl(value, instance, parent.first, propertyName,
        ...
        template<typename T>
        static T GetValueImpl(void* instance, const std::string& className,
            const std::string& propertyName, size_t offset)
        {
            auto classByType = ReflectionSystem::GetInstance().m_ClassesByType.find(className);
            if (classByType != ReflectionSystem::GetInstance().m_ClassesByType.end())
            {
                ...
                for (const auto& parent : classByType->second.m_Parents)
                {
                    return GetValueImpl<T>(instance, parent.first, propertyName,
                        offset + parent.second);
                }
        ...
        template<typename T>
        static T GetValue(void* instance, const std::string& className,
            const std::string& propertyName)
        {
...
private: \
ReflectionSystem::ReflectionWrapper ___ReflectionWrapper_##_name = ReflectionSystem::ReflectionWrapper([baseClass = this] \
{ \
...
#define REGISTER_PARENT_CLASS(_type) private: ReflectionSystem::ParentInitializer ___ParentInitializer_##_type = ReflectionSystem::ParentInitializer(GetTypeName<decltype(*this)>(), GetTypeName<_type>(), (size_t)(_type*)(this) - (size_t)this);
