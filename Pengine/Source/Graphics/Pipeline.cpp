...
std::shared_ptr<Pipeline> Pipeline::Create(const Pipeline::CreateInfo& pipelineCreateInfo)
{
...
Pipeline::Pipeline(const Pipeline::CreateInfo& pipelineCreateInfo)
    : createInfo(pipelineCreateInfo)
{
    if (!pipelineCreateInfo.uniformBindings.empty())
    {
        std::shared_ptr<UniformLayout> layout = UniformLayout::Create(pipelineCreateInfo.uniformBindings);
        ...
        for (const auto& [location, binding] : layout->GetBindingsByLocation())
        {
            if (binding.type == UniformLayout::Type::BUFFER)
            {
                ...
                auto buffer = Buffer::Create(bufferSize, 1, { Buffer::Usage::TRANSFER_SRC, Buffer::Usage::UNIFORM_BUFFER });
                buffer->Map();
    ...
    if (!pipelineCreateInfo.childUniformBindings.empty())
    {
        ...
        for (const auto& [location, binding] : m_ChildUniformLayout->GetBindingsByLocation())
        {
            if (binding.type == UniformLayout::Type::BUFFER)
            {
                ...
                // Performance issue can appear due to Buffer::Usage::TRANSFER_SRC.
                auto buffer = Buffer::Create(bufferSize, MAX_MATERIALS, { Buffer::Usage::TRANSFER_SRC, Buffer::Usage::UNIFORM_BUFFER });
                buffer->Map();
                m_BuffersByName[binding.name] = buffer;
            }
        }
